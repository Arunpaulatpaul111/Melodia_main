...
<body class="">
          ...
          <div class="row">
              <div class="card">
                <div class="card-header card-header-primary">
                  <h4 class="card-title">View the course status</h4>
                    ...
                    <div class="row">
                      <div class="col-md-12">
                        <div class="form-group">
                          <input type="text" name="instruId" id="instruId" style="display:none;" value="<?php echo $instruId;?>"/>
                          <label class="bmd-label-floating">Select Schedule</label>
                          <select id="lCourse" name="lCourse" onchange="doList(this.value)" class="form-control" required>
                            <option value="" >Select Schedule</option>
                              <?php
                                $sql3="SELECT * FROM tbl_schedule";
                                $res3=mysqli_query($con,$sql3) or die(mysqli_error($con));
                                while($row3=mysqli_fetch_array($res3)){
                                  echo"<option value='".$row3['batch_id']."' class='input100'>".$row3['batchName']."</option>";
                                }
                              ?>
                          </select>
                          <label style="display:nne; color:red" id="status_error"></label>
                        </div>
                      </div>
                    </div>
                  </form>
  ...
  <script>
    // -----------list the course status---------
    function doList(val) {
      var iId=document.getElementById('instruId').value;
      $.ajax({
        type: "POST",
        ...
        success: function(data){
          if(data==1){
            $("#status_error").html('No data found..!').fadeIn().delay(4000).fadeOut();
            $("#aList").hide();
          } else{
            $("#aList").show();
            $("#dataList").html(data);
